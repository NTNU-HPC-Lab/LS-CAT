...
cudaMalloc(&templateFeatures, XSIZE*YSIZE);
int colNumberM = 1;
int rowNumberN = 1;
float *l1SumTable = NULL;
cudaMalloc(&l1SumTable, XSIZE*YSIZE);
float *l2SumTable = NULL;
cudaMalloc(&l2SumTable, XSIZE*YSIZE);
float *lxSumTable = NULL;
cudaMalloc(&lxSumTable, XSIZE*YSIZE);
float *lySumTable = NULL;
cudaMalloc(&lySumTable, XSIZE*YSIZE);
int Kx = 1;
int Ky = 1;
float *differences = NULL;
cudaMalloc(&differences, XSIZE*YSIZE);
int iXSIZE= XSIZE;
int iYSIZE= YSIZE;
