...
cudaMalloc(&acExpDetImages, XSIZE*YSIZE);
const int *aiDetStartIdx = NULL;
cudaMalloc(&aiDetStartIdx, XSIZE*YSIZE);
const float *afDetInfo = NULL;
cudaMalloc(&afDetInfo, XSIZE*YSIZE);
const int iNDet = 1;
const int iNRot = 1;
const int *aiDetIndex = NULL;
cudaMalloc(&aiDetIndex, XSIZE*YSIZE);
const int *aiRotN = NULL;
cudaMalloc(&aiRotN, XSIZE*YSIZE);
const int *aiJExp = NULL;
cudaMalloc(&aiJExp, XSIZE*YSIZE);
const int *aiKExp = NULL;
cudaMalloc(&aiKExp, XSIZE*YSIZE);
int const iNPeak = 1;
int iXSIZE= XSIZE;
int iYSIZE= YSIZE;
