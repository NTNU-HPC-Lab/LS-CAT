...
cudaMalloc(&d_new_peak_list_DM, XSIZE*YSIZE);
unsigned int *d_new_peak_list_TS = NULL;
cudaMalloc(&d_new_peak_list_TS, XSIZE*YSIZE);
unsigned int *d_new_peak_list_BW = NULL;
cudaMalloc(&d_new_peak_list_BW, XSIZE*YSIZE);
float *d_new_peak_list_SNR = NULL;
cudaMalloc(&d_new_peak_list_SNR, XSIZE*YSIZE);
unsigned int *d_peak_list_DM = NULL;
cudaMalloc(&d_peak_list_DM, XSIZE*YSIZE);
unsigned int *d_peak_list_TS = NULL;
cudaMalloc(&d_peak_list_TS, XSIZE*YSIZE);
unsigned int *d_peak_list_BW = NULL;
cudaMalloc(&d_peak_list_BW, XSIZE*YSIZE);
float *d_peak_list_SNR = NULL;
cudaMalloc(&d_peak_list_SNR, XSIZE*YSIZE);
unsigned int nElements = 1;
unsigned int max_distance = 1;
int nLoops = 1;
int max_list_pos = 1;
int *gmem_pos = NULL;
cudaMalloc(&gmem_pos, XSIZE*YSIZE);
int iXSIZE= XSIZE;
int iYSIZE= YSIZE;
