...
cudaMalloc(&vectsa, XSIZE*YSIZE);
size_t na = 1;
const float *vectsb = NULL;
cudaMalloc(&vectsb, XSIZE*YSIZE);
size_t nb = 1;
size_t dim = 2;
const float *numPairs = NULL;
cudaMalloc(&numPairs, XSIZE*YSIZE);
const float *means = NULL;
cudaMalloc(&means, XSIZE*YSIZE);
const float *sds = NULL;
cudaMalloc(&sds, XSIZE*YSIZE);
float *correlations = NULL;
cudaMalloc(&correlations, XSIZE*YSIZE);
int iXSIZE= XSIZE;
int iYSIZE= YSIZE;
