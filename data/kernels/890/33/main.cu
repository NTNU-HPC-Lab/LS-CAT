...
cudaMalloc(&gpuDelays, XSIZE*YSIZE);
double lo = 1;
double sampletime = 1;
int fftsamples = 1;
int fftchannels = 1;
...
cudaMalloc(&rotationPhaseInfo, XSIZE*YSIZE);
int *sampleShifts = NULL;
cudaMalloc(&sampleShifts, XSIZE*YSIZE);
float *fractionalSampleDelays = NULL;
cudaMalloc(&fractionalSampleDelays, XSIZE*YSIZE);
int iXSIZE= XSIZE;
int iYSIZE= YSIZE;
