...
cudaMalloc(&cum_row_indexes, XSIZE*YSIZE);
unsigned int *column_indexes = NULL;
cudaMalloc(&column_indexes, XSIZE*YSIZE);
int *matrix_data = NULL;
cudaMalloc(&matrix_data, XSIZE*YSIZE);
unsigned int *in_infections = NULL;
cudaMalloc(&in_infections, XSIZE*YSIZE);
unsigned int *out_infections = NULL;
cudaMalloc(&out_infections, XSIZE*YSIZE);
unsigned int rows = 1;
int iXSIZE= XSIZE;
int iYSIZE= YSIZE;
