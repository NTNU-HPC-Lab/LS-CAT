...
cudaMalloc(&corelation, XSIZE*YSIZE);
float *miu_x = NULL;
cudaMalloc(&miu_x, XSIZE*YSIZE);
float *miu_y = NULL;
cudaMalloc(&miu_y, XSIZE*YSIZE);
float *stdx = NULL;
cudaMalloc(&stdx, XSIZE*YSIZE);
float *stdy = NULL;
cudaMalloc(&stdy, XSIZE*YSIZE);
int *ikj = NULL;
cudaMalloc(&ikj, XSIZE*YSIZE);
float *dif_variance = NULL;
cudaMalloc(&dif_variance, XSIZE*YSIZE);
int max = 1;
float sum = 1;
