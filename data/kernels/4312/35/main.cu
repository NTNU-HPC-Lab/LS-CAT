...
cudaMalloc(&diagonal_blks, XSIZE*YSIZE);
const float *nondiagonal_blks = NULL;
cudaMalloc(&nondiagonal_blks, XSIZE*YSIZE);
const int *csr_rowptr = NULL;
cudaMalloc(&csr_rowptr, XSIZE*YSIZE);
const unsigned *blkrow_offset = NULL;
cudaMalloc(&blkrow_offset, XSIZE*YSIZE);
float *JtJ_data = NULL;
cudaMalloc(&JtJ_data, XSIZE*YSIZE);
int iXSIZE= XSIZE;
int iYSIZE= YSIZE;
